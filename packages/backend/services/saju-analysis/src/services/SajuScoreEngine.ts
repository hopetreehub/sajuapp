...
export interface ScoreBreakdown {
  saju_influence: number        // 사주 영향도
  temporal_influence: number    // 시점별 영향도
  category_fitness: number      // 카테고리 적합도
  total: number                // 최종 점수
}
...
export class SajuScoreEngine {
  ...
  async calculateComprehensiveScores(
    ...
    categories: any,
    db: any
  ): Promise<ComprehensiveScores> {
    const positiveScores = new Map<string, CategoryScore>()
    const negativeScores = new Map<string, CategoryScore>()
  ...
  private calculateCategoryScore(
    ...
    // 일운 점수 계산
    const dailyScore = this.calculateTemporalScore(
      userSaju,
      currentPillars.current_day,
      baseScore,
      0.2 // 20% 가중치
    )
    
    // 월운 점수 계산
    const monthlyScore = this.calculateTemporalScore(
      userSaju,
      currentPillars.current_month,
      baseScore,
      0.3 // 30% 가중치
    )
    
    // 세운 점수 계산
    const yearlyScore = this.calculateTemporalScore(
      userSaju,
      currentPillars.current_year,
      baseScore,
      0.5 // 50% 가중치
    )
  ...
  private calculateTemporalScore(
    ...
    // 천간 상호작용
    const heavenlyInteraction = this.evaluateHeavenlyInteraction(
    ...
    // 지지 상호작용
    const earthlyInteraction = this.evaluateEarthlyInteraction(
   ...
   * 오행 균형도 평가
   */
  private evaluateElementBalance(elements: any): number {
    ...
    if (stdDev < 0.5) return 30  // 완벽한 균형
    if (stdDev < 1.0) return 20  // 좋은 균형
    if (stdDev < 1.5) return 10  // 보통 균형
    if (stdDev < 2.0) return 0   // 약간 불균형
    return -10 // 심한 불균형
  }
   ...
   * 십성 평가
   */
  private evaluateTenGods(tenGods: string[], categoryName: string): number {
    let score = 0
    
    // 카테고리별 유리한 십성 매칭
    const favorableGods: { [key: string]: string[] } = {
      '게임': ['식신', '상관', '편재'],
      '연예': ['식신', '상관', '정관'],
      '체능': ['비견', '겁재', '편관'],
      '문학': ['정인', '편인', '식신'],
   ...
   * 천간 상호작용 평가
   */
  private evaluateHeavenlyInteraction(userStem: string, currentStem: string): number {
    // 천간 상생상극 관계 판단
    ...
    if (!userElement || !currentElement) return 0
   ...
   * 지지 상호작용 평가
   */
  private evaluateEarthlyInteraction(userBranch: string, currentBranch: string): number {
    // 지지 충합 관계 판단
    const clashPairs: { [key: string]: string } = {
      '자': '오', '축': '미', '인': '신', '묘': '유', '진': '술', '사': '해',
      '오': '자', '미': '축', '신': '인', '유': '묘', '술': '진', '해': '사'
    }
    
    const harmonyPairs: { [key: string]: string } = {
   ...
   * 카테고리별 특수 보정
   */
  private getCategorySpecificAdjustment(categoryName: string, saju: SajuData): number {
    let adjustment = 0
    
    // 카테고리별 특수 조건
    switch(categoryName) {
      case '게임':
        if (saju.five_elements.water > 1.5) adjustment += 5
        if (saju.five_elements.metal > 1.5) adjustment += 5
        break
      case '체능':
        if (saju.five_elements.fire > 1.5) adjustment += 5
        if (saju.five_elements.wood > 1.5) adjustment += 5
        break
      case '문학':
        if (saju.five_elements.water > 1.5) adjustment += 5
        if (saju.ten_gods.includes('정인')) adjustment += 5
        break
      case '교통사고':
        if (saju.ten_gods.includes('편관')) adjustment += 10
        if (saju.ten_gods.includes('겁재')) adjustment += 10
        break
    }
   ...
   * 개별 항목 점수 계산
   */
  private calculateItemScore(saju: SajuData, item: any, type: string): number {
    let score = 50 // 기본 점수
   ...
   * 신뢰도 계산
   */
  private calculateConfidence(saju: SajuData, item: any): number {
    // 기본 신뢰도
    let confidence = 0.5
   ...
   * 항목과 오행 매칭
   */
  private matchItemElement(saju: SajuData, itemName: string): number {
    // 항목명에서 오행 연관성 추출
    ...
    for (const [keyword, element] of Object.entries(elementKeywords)) {
      if (itemName.includes(keyword)) {
        const elementValue = (saju.five_elements as any)[element]
        if (elementValue > 1.5) return 2
        if (elementValue > 1.0) return 1
      }
    }
   ...
   * 카테고리 데이터 로드
   */
  private async loadCategories(db: any, type: 'positive' | 'negative'): Promise<any> {
  ...
  private generateSummary(
    positiveScores: Map<string, CategoryScore>,
    negativeScores: Map<string, CategoryScore>
  ): any {
    // 전체 운세 계산
    let totalPositive = 0
    let totalNegative = 0
    ...
    if (dailyAvg > yearlyAvg + 10) trend = 'rising'
    else if (dailyAvg < yearlyAvg - 10) trend = 'declining'
