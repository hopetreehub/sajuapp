...
console.log('💃 주능 - 무용 카테고리 중복 정리 시작...\n');
db.serialize(() => {
    ...
    `, (err, dances) => {
            if (err) {
                console.error('❌ 무용 항목 분석 실패:', err);
                db.run('ROLLBACK');
                db.close();
                return;
            }
            console.log(`💃 무용 카테고리 현재 상태: ${dances.length}개 항목`);
            // 이름별로 그룹화
            const groups = {};
            dances.forEach(item => {
                if (!groups[item.name])
            ...
            console.log(`\n2️⃣ 무용 카테고리 ${toDelete.length}개 중복 항목 삭제 중...`);
            // 중복 항목들 삭제
      ...
      `, toDelete, (err) => {
                if (err) {
                    console.error('❌ 무용 중복 삭제 실패:', err);
                    db.run('ROLLBACK');
                    db.close();
                    return;
                }
                console.log('✅ 무용 중복 삭제 완료!');
                // 최종 확인
                console.log('\n3️⃣ 무용 카테고리 최종 검증...');
                db.all(`
        ...
        `, (err, stillDuplicates) => {
                    ...
                    if (stillDuplicates.length > 0) {
                        ...
                        console.log('✅ 무용 카테고리 중복 완전 제거 완료!');
                    }
                    // 전체 무용 항목 목록 표시
                    db.all(`
            SELECT mc.name
...
//# sourceMappingURL=fix-dance-category.js.map