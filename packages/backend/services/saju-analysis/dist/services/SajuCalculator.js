...
class SajuCalculator {
     ...
     * 메인 사주 계산 함수
     */
    async calculateSaju(birthDate, birthTime, isLunar = false) {
     ...
     * 오행 분석
     */
    analyzeFiveElements(yearPillar, monthPillar, dayPillar, hourPillar) {
     ...
     * 오행 상생상극 관계 분석
     */
    getElementRelationship(source, target) {
        ...
        if (source === target)
            return 'same';
        if (generationCycle[source] === target)
            return 'generate';
        if (destructionCycle[source] === target)
            return 'destroy';
        if (generationCycle[target] === source)
            return 'support';
        if (destructionCycle[target] === source)
            return 'restrain';
        return 'neutral';
    }
    /**
     * 십성명 결정
     */
    getTenGodName(relationship, targetStem, dayMaster) {
     ...
     * 일주 강약 분석
     */
    analyzeStrength(dayPillar, monthPillar, fiveElements, month) {
     ...
     * 계절별 오행 강약
     */
    getSeasonalStrength(element, season) {
        var _a;
        ...
        return ((_a = strengthMatrix[season]) === null || _a === void 0 ? void 0 : _a[element]) || 1.0;
    }
    /**
     * 음력-양력 변환 (간단한 근사치)
     * 실제 구현에서는 전문 라이브러리 사용 권장
     */
    convertLunarToSolar(year, month, day) {
     ...
     */
    calculateCurrentTimePillars(targetDate) {
     ...
     */
    async analyzeTemporalSaju(birthDate, birthTime, isLunar = false, targetDate) {
     ...
     */
    analyzeTemporalInteractions(personalSaju, currentPillars) {
     ...
     */
    analyzePillarInteraction(personalStem, currentStem, period) {
     ...
     */
    calculateFortuneTrends(personalSaju, currentPillars, interactions) {
        // 각 상호작용을 점수로 변환
     ...
     * 상호작용 텍스트를 점수로 변환
     */
    interactionToScore(interaction) {
        if (interaction.includes('생조관계'))
            return 70; // 매우 좋음
        if (interaction.includes('동기상조'))
            return 50; // 좋음
        if (interaction.includes('인수관계'))
            return 40; // 괜찮음  
        if (interaction.includes('중성관계'))
            return 0; // 보통
        if (interaction.includes('관살관계'))
            return -30; // 어려움
        if (interaction.includes('극제관계'))
            return -50; // 매우 어려움
        return 0; // 기본값
    }
    /**
     * 현재 시점의 기둥 계산
     */
    async getCurrentTimePillars(targetDate) {
     ...
     */
    async analyzeEnhancedTemporalSaju(birthDate, birthTime, isLunar = false, targetDate, db) {
        ...
        // 3. 시점별 권장사항 생성
        const recommendations = this.generateTemporalRecommendations(basicAnalysis, positiveCategories, negativeCategories);
     ...
     */
    async loadTemporalCategories(db, type, analysis) {
        const categories = {};
        try {
     ...
     * 시점별 가중치 조정 계산
     */
    calculateTemporalModifier(analysis, itemName, type) {
        // 기본 가중치
        let modifier = 1.0;
        // 운세 점수에 따른 조정
        const avgFortune = (analysis.fortune_trends.current_year_fortune +
            analysis.fortune_trends.current_month_fortune +
            analysis.fortune_trends.current_day_fortune) / 3;
        if (type === 'positive') {
            // 주능의 경우 운세가 좋을 때 강화
     ...
     * 시점별 권장사항 생성
     */
    generateTemporalRecommendations(analysis, positiveCategories, negativeCategories) {
        const favorable = [];
        const cautions = [];
        // 주능에서 가중치 높은 항목 추출
        ...
        if (avgFortune > 30)
            timing = '매우 좋은 시기';
        else if (avgFortune > 0)
            timing = '좋은 시기';
        else if (avgFortune > -30)
            timing = '주의 필요한 시기';
        else
            timing = '신중해야 할 시기';
        return {
