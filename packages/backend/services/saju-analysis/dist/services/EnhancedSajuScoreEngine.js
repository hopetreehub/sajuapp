...
exports.EnhancedSajuScoreEngine = void 0;
class EnhancedSajuScoreEngine {
    constructor() {
        // 오행별 적성 분야 매핑
        this.elementAptitudes = {
            '목': {
     ...
     * 향상된 카테고리별 점수 계산
     */
    calculateEnhancedCategoryScore(userSaju, currentPillars, categoryName, items, type) {
        // 1. 오행 친화도 계산 (40점)
     ...
     * 오행 친화도 계산 (0-40점)
     */
    calculateElementAffinity(saju, categoryName) {
        ...
        for (const [element, aptitude] of Object.entries(this.elementAptitudes)) {
            if (aptitude.categories.includes(categoryName) ||
                ...
                // 오행 강도에 따른 보너스
                const elementStrength = saju.five_elements[this.getElementKey(element)];
     ...
     * 십성 조화도 계산 (0-30점)
     */
    calculateTenGodsHarmony(saju, categoryName) {
        let harmony = 0;
        for (const god of saju.ten_gods) {
            const aptitude = this.tenGodsAptitudes[god];
            if (aptitude && aptitude.suitable.some(field => categoryName.includes(field) || field.includes(categoryName))) {
     ...
     * 계절 보너스 계산 (0-10점)
     */
    calculateSeasonalBonus(saju, categoryName) {
    ...
    calculateTemporalScore(userSaju, currentPillar, baseScore, weight) {
        let modifier = 0;
        // 천간 상호작용
        const heavenlyRelation = this.getHeavenlyRelation(userSaju.day_pillar.heavenly, currentPillar.heavenly);
        modifier += this.getRelationScore(heavenlyRelation) * weight;
        // 지지 상호작용  
        const earthlyRelation = this.getEarthlyRelation(userSaju.day_pillar.earthly, currentPillar.earthly);
     ...
     * 항목별 상세 분석
     */
    calculateDetailedItems(saju, items, categoryName, type) {
     ...
     * 개별 항목 점수 계산
     */
    calculateIndividualItemScore(saju, item, categoryName) {
        let score = 50; // 기본 점수
        // 사주 가중치 적용
        if (item.saju_weight) {
     ...
     * 신뢰도 계산
     */
    calculateConfidenceLevel(saju, categoryName) {
    ...
    isGeneratingRelation(source, target) {
        const generation = {
            '목': '화', '화': '토', '토': '금', '금': '수', '수': '목'
        };
        return generation[source] === target;
    }
    isSupportingRelation(source, target) {
        const support = {
            '화': '목', '토': '화', '금': '토', '수': '금', '목': '수'
        };
        return support[source] === target;
    }
    isRelatedActivity(categoryName, activity) {
        const relationMap = {
            '야외활동': ['체능', '체육', '스포츠'],
            '사교활동': ['연예', '영업', 'MC'],
            '수확관련': ['농업', '관리', '완성'],
    ...
    getHeavenlyRelation(userStem, currentStem) {
        if (userStem === currentStem)
            return '동일';
        const userElement = this.getElementFromStem(userStem);
        const currentElement = this.getElementFromStem(currentStem);
        if (this.isGeneratingRelation(userElement, currentElement))
            return '상생';
        if (this.isGeneratingRelation(currentElement, userElement))
            return '피생';
        return '중성';
    }
    getEarthlyRelation(userBranch, currentBranch) {
        // 지지 충합 관계 (간단 구현)
        const clashes = {
    ...
    getRelationScore(relation) {
        const scores = {
            '상생': 15, '피생': 10, '동일': 8, '중성': 0, '충': -15
        };
        return scores[relation] || 0;
    }
    getAffinityReason(saju, item, categoryName) {
        const reasons = [];
        // 오행 이유
    ...
    calculateItemConfidence(saju, item) {
        let confidence = 0.6;
        if (item.saju_weight > 1.5)
            confidence += 0.2;
        if (saju.strength.day_master_strength > 6)
            confidence += 0.1;
        if (this.calculateElementBalance(saju.five_elements) > 70)
            confidence += 0.1;
        return Math.min(1.0, confidence);
    }
    getItemElementBonus(saju, itemName) {
        // 항목명에서 오행 연관성 찾기
        ...
        for (const [element, keywords] of Object.entries(elementKeywords)) {
            if (keywords.some(keyword => itemName.includes(keyword))) {
                const elementValue = saju.five_elements[this.getElementKey(element)];
                return Math.min(15, elementValue * 8);
            }
        }
        return 0;
    }
    getItemTenGodBonus(saju, itemName) {
        // 십성별 항목 매칭
        for (const god of saju.ten_gods) {
            const aptitude = this.tenGodsAptitudes[god];
            if (aptitude) {
    ...
    getCategorySpecificBonus(saju, itemName, categoryName) {
        // 카테고리별 특수 보너스 로직
        let bonus = 0;
        switch (categoryName) {
            case '게임':
                ...
                break;
            case '연예':
                if (saju.ten_gods.includes('식신') || saju.ten_gods.includes('상관'))
                    bonus += 10;
                break;
            case '체능':
    ...
    calculateSajuCompleteness(saju) {
        let completeness = 0;
        // 사주 기둥 완성도
...
exports.EnhancedSajuScoreEngine = EnhancedSajuScoreEngine;
//# sourceMappingURL=EnhancedSajuScoreEngine.js.map