...
exports.AptitudeAnalyzer = void 0;
class AptitudeAnalyzer {
     ...
     * 메인 적성 분석 함수
     */
    async analyzeAptitude(sajuData) {
     ...
     * 주능 (긍정적 적성) 분석
     */
    async analyzePositiveAptitudes(sajuData, categories) {
        const result = {};
     ...
     * 주흉 (주의사항) 분석
     */
    async analyzeNegativeWarnings(sajuData, categories) {
        const result = {};
        for (const [categoryName, categoryData] of Object.entries(categories)) {
     ...
     * 카테고리별 분석
     */
    analyzeCategory(sajuData, categoryName, categoryData, type) {
    ...
    analyzeGaming(sajuData, items) {
        ...
        if (waterStrength > 1.5 || metalStrength > 1.5) {
            suitable.push(...items.filter(item => ['FPS게임', '액션게임', '스포츠게임'].includes(item.name)));
        }
        if (sajuData.five_elements.earth > 1.2) {
            suitable.push(...items.filter(item => ['시뮬레이션게임', '롤플레잉게임'].includes(item.name)));
        }
     ...
     * 과목 분야 분석
     */
    analyzeSubjects(sajuData, items) {
        const suitable = [];
        const dayMaster = this.getElementFromStem(sajuData.day_pillar.heavenly);
        switch (dayMaster) {
            case 'wood':
                suitable.push(...items.filter(item => ['국어', '영어', '음악', '미술'].includes(item.name)));
                break;
            case 'fire':
                suitable.push(...items.filter(item => ['체육', '음악', '미술'].includes(item.name)));
                break;
            case 'earth':
                suitable.push(...items.filter(item => ['사회', '도덕', '한국사'].includes(item.name)));
                break;
            case 'metal':
                suitable.push(...items.filter(item => ['수학', '과학', '기술'].includes(item.name)));
                break;
            case 'water':
                suitable.push(...items.filter(item => ['한문', '국어', '영어'].includes(item.name)));
                break;
        }
    ...
    analyzeDance(sajuData, items) {
        ...
        if (fireStrength > 1.0 && woodStrength > 1.0) {
            suitable.push(...items.filter(item => ['현대무용', '대중무용', '스포츠댄스'].includes(item.name)));
        }
        if (sajuData.five_elements.earth > 1.2) {
            suitable.push(...items.filter(item => ['전통무용', '민속무용'].includes(item.name)));
        }
        return suitable.slice(0, 3);
    }
    /**
     * 문학 분야 분석
     */
    analyzeLiterature(sajuData, items) {
        const suitable = [];
        const waterStrength = sajuData.five_elements.water || 0;
        if (waterStrength > 1.5) {
            suitable.push(...items.filter(item => ['소설가', '시인', '시나리오작가'].includes(item.name)));
        }
        if (sajuData.ten_gods.includes('상관') || sajuData.ten_gods.includes('식신')) {
            suitable.push(...items.filter(item => ['방송작가', '라디오작가', '작사가'].includes(item.name)));
        }
        return suitable.slice(0, 3);
    }
    /**
     * 미술 분야 분석
     */
    analyzeArts(sajuData, items) {
        const suitable = [];
        const fireStrength = sajuData.five_elements.fire || 0;
        const earthStrength = sajuData.five_elements.earth || 0;
        if (fireStrength > 1.2) {
            suitable.push(...items.filter(item => ['서양화', '디자인', '시각디자인'].includes(item.name)));
        }
        if (earthStrength > 1.2) {
            suitable.push(...items.filter(item => ['조소', '공예', '인테리어'].includes(item.name)));
        }
        return suitable.slice(0, 4);
    }
    /**
     * 연예 분야 분석
     */
    analyzeEntertainment(sajuData, items) {
        const suitable = [];
        const fireStrength = sajuData.five_elements.fire || 0;
        if (fireStrength > 1.5) {
            suitable.push(...items.filter(item => ['가수', '연기자', '드라마배우'].includes(item.name)));
        }
        if (sajuData.ten_gods.includes('상관')) {
            suitable.push(...items.filter(item => ['개그맨', 'MC', '뮤지컬배우'].includes(item.name)));
        }
        return suitable.slice(0, 3);
    }
    /**
     * 음악 분야 분석
     */
    analyzeMusic(sajuData, items) {
        const suitable = [];
        const metalStrength = sajuData.five_elements.metal || 0;
        const waterStrength = sajuData.five_elements.water || 0;
        if (metalStrength > 1.0) {
            suitable.push(...items.filter(item => ['건반악기', '관악기', '작곡'].includes(item.name)));
        }
        if (waterStrength > 1.0) {
            suitable.push(...items.filter(item => ['보컬', '대중음악', '성악'].includes(item.name)));
        }
        return suitable.slice(0, 3);
    }
    /**
     * 전공 분야 분석
     */
    analyzeMajors(sajuData, items) {
        const suitable = [];
        const dayMaster = this.getElementFromStem(sajuData.day_pillar.heavenly);
        switch (dayMaster) {
            case 'wood':
                suitable.push(...items.filter(item => ['어문인문학계', '사범계', '예체능계'].includes(item.name)));
                break;
            case 'fire':
                suitable.push(...items.filter(item => ['예체능계', '사회과학계'].includes(item.name)));
                break;
            case 'earth':
                suitable.push(...items.filter(item => ['사회과학계', '생활과학계', '농생명과학계'].includes(item.name)));
                break;
            case 'metal':
                suitable.push(...items.filter(item => ['공학계', '자연과학계', '의치악계'].includes(item.name)));
                break;
            case 'water':
                suitable.push(...items.filter(item => ['법정계', '어문인문학계'].includes(item.name)));
                break;
        }
        return suitable.slice(0, 3);
    }
     ...
     * 체능 분야 분석
     */
    analyzeSports(sajuData, items) {
        const suitable = [];
        const fireStrength = sajuData.five_elements.fire || 0;
        const woodStrength = sajuData.five_elements.wood || 0;
        if (fireStrength > 1.5) {
            suitable.push(...items.filter(item => ['축구', '농구', '배구', '야구'].includes(item.name)));
        }
        if (woodStrength > 1.0) {
            suitable.push(...items.filter(item => ['골프', '테니스', '배드민턴'].includes(item.name)));
        }
        if (sajuData.five_elements.water > 1.0) {
            suitable.push(...items.filter(item => ['수영', '수상스키', '요트'].includes(item.name)));
        }
     ...
     * 교통사고 위험 분석
     */
    analyzeTrafficRisks(sajuData, items) {
        const risks = [];
        const metalStrength = sajuData.five_elements.metal || 0;
        if (metalStrength > 2.0) {
            risks.push(...items.filter(item => ['충돌사고', '과속사고', '접촉사고'].includes(item.name)));
        }
        if (sajuData.strength.day_master_strength < 1.5) {
            risks.push(...items.filter(item => ['졸음운전', '신호위반'].includes(item.name)));
        }
     ...
     * 법적 사건 위험 분석
     */
    analyzeLegalRisks(sajuData, items) {
        const risks = [];
        if (sajuData.ten_gods.includes('편관') || sajuData.ten_gods.includes('상관')) {
            risks.push(...items.filter(item => ['소송', '명예훼손', '폭행'].includes(item.name)));
        }
     ...
     * 일반 사고 위험 분석
     */
    analyzeAccidentRisks(sajuData, items) {
        const risks = [];
        if (sajuData.strength.day_master_strength < 1.5) {
            risks.push(...items.filter(item => ['분실', '손실', '언쟁'].includes(item.name)));
        }
        return risks.slice(0, 2);
    }
    /**
     * 도로별 위험 분석
     */
    analyzeRoadRisks(sajuData, items) {
        const risks = [];
        if (sajuData.five_elements.metal > 1.5) {
            risks.push(...items.filter(item => ['고속도로', '사거리', '고가도로'].includes(item.name)));
        }
        return risks.slice(0, 3);
    }
    /**
     ...
     * 위험도 계산
     */
    calculateRiskLevel(itemCount, confidence) {
     ...
     * 전체 신뢰도 계산
     */
    calculateOverallConfidence(sajuData, positive, negative) {
        let baseConfidence = 0.7;
        // 오행 균형도
        const elements = Object.values(sajuData.five_elements);
     ...
     * 분석 요약 생성
     */
    generateSummary(sajuData, positive, negative) {
...
exports.AptitudeAnalyzer = AptitudeAnalyzer;
//# sourceMappingURL=AptitudeAnalyzer.js.map