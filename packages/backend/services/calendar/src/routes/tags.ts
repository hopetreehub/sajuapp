...
router.post('/events/:eventId/tags', async (req: Request, res: Response) => {
  try {
    const userId = req.headers['x-user-id'] || 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11'
    ...
    logger.error('Failed to add tags to event:', error)
    res.status(500).json({ error: 'Failed to add tags to event' })
  }
})

// GET /api/events/:eventId/tags - 이벤트의 태그 목록 조회
router.get('/events/:eventId/tags', async (req: Request, res: Response) => {
  try {
    const userId = req.headers['x-user-id'] || 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11'
    const { eventId } = req.params
...
router.delete('/events/:eventId/tags/:tagId', async (req: Request, res: Response) => {
  try {
    const userId = req.headers['x-user-id'] || 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11'
