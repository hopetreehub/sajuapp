...
exports.calculateCompleteSaju = calculateCompleteSaju;
// 60갑자 배열
const SIXTY_CYCLE = [
...
function calculateHourPillar(year, month, day, hour, _minute) {
    ...
    if (adjustedHour === 23) {
        ...
        dayPillar = calculateDayPillar(nextDay.getFullYear(), nextDay.getMonth() + 1, nextDay.getDate());
    }
    else {
...
// 오행 밸런스 계산
function calculateOhHaengBalance(year, month, day, time) {
    const elements = {
        '목': 0, '화': 0, '토': 0, '금': 0, '수': 0
    };
    // 천간의 오행
...
// 완전한 사주 계산
function calculateCompleteSaju(year, month, day, hour, minute, _isLunar = false) {
    ...
    const ohHaengBalance = calculateOhHaengBalance(yearPillar, monthPillar, dayPillar, hourPillar);
    return {
        year: yearPillar,
        month: monthPillar,
        day: dayPillar,
...
//# sourceMappingURL=accurateSajuCalculator.js.map