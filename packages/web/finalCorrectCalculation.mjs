// 정확한 기준일(1900/1/1 = 병오일)로 최종 사주 계산
...
console.log('=== 정확한 기준일로 최종 사주 계산 ===\n');

// 기준일 검증
console.log('기준일 검증:');
const baseTest = calculateDayPillar(1900, 1, 1);
console.log('1900년 1월 1일:', baseTest, baseTest === '병오' ? '✅' : '❌');

// 웹 검색 결과 검증
const webTest = calculateDayPillar(1971, 11, 17);
console.log('1971년 11월 17일:', webTest, webTest === '무인' ? '✅' : '❌');

// 1971년 11월 17일 04시 최종 정확한 계산
console.log('\n1971년 11월 17일 04시 최종 정확한 계산:');
...
console.log('\n🎯 최종 정확한 사주:', `${yearPillar} ${monthPillar} ${dayPillar} ${hourPillar}`);
...
testCases.forEach(({year, month, day, desc}) => {
...
console.log('기준 index 42(병오) + ' + dayDiff + ' = ' + ((42 + dayDiff) % 60));
console.log('결과 index:', (42 + dayDiff) % 60, '→', SIXTY_CYCLE[(42 + dayDiff) % 60]);