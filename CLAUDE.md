...
### 포트 구성 정책
⚠️ **절대 규칙: 모든 서비스는 무조건 4000 포트만 사용**

- **통합 포트**: 4000 (모든 서비스 통합, 절대 불변)
- **프론트엔드**: 4000 포트 사용
- **백엔드 서비스**: 4000 포트의 서브 경로로 프록시
- **다른 포트 사용 금지**: 4001, 4002, 3000 등 일체 사용 불가
- **이유**: 포트 관리 단순화 및 일관성 유지
- **예외**: 절대 없음. 임시 포트도 사용 금지

### 포트 4000 절대 정책
🚫 **금지사항**: 4004, 4005, 3000 등 다른 포트 절대 사용 불가
✅ **필수사항**: 포트 충돌 시 해당 프로세스 종료 후 4000 포트 사용

### 서버 실행 명령어
```bash
# 통합 서버 실행 (포트 4000만 사용)
cd packages/web && npx vite --port 4000

# Vite 프록시 설정으로 백엔드 연결
